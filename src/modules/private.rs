...
impl Private {
    ...
    pub async fn create_order(&self, user_params: ApiOrderParams) -> Result<OrderResponse> {
        ...
        let parameters = ApiOrder {
            ...
            size: user_params.size.to_string(),
            price: user_params.price.to_string(),
    ...
    async fn request<T: for<'de> Deserialize<'de>, V: Serialize>(
        ...
        let req_builder = req_builder
            ...
            .header("DYDX-API-KEY", self.api_key_credentials.key.clone())
            .header(
                "DYDX-PASSPHRASE",
                self.api_key_credentials.passphrase.clone(),
            )
            .query(&parameters);
